...
func (topology *provider) Init(ctx servicehub.Context) (err error) {
	...
	// translator
	topology.i18n = ctx.Service("i18n").(i18n.I18n).Translator("topology")
	...
	c := ctx.Service("cassandra").(cassandra.Cassandra)
	...
	routes := ctx.Service("http-server", telemetry.HttpMetric(), interceptors.Recover(topology.Log)).(httpserver.Router)
